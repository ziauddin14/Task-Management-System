...
function WhatsAppShareButton({ file }) {
  if (!file) return null;
  ...
  if (!canShareFile) {
    return <p className="mt-2 text-sm text-gray-500">Faisal ki gayi file WhatsApp Desktop/Web mein manually attach kar dein</p>;
  }

  async function handleShare() {
    try {
      await navigator.share({ files: [file], title: 'Task Report' });
    } catch {
